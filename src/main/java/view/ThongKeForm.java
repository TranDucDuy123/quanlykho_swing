...
public class ThongKeForm extends javax.swing.JInternalFrame {
    ...
    private void loadDataToTableThongKeProduct(ArrayList<ThongKeProduct> thongKe) {
        try {
                    DefaultTableModel tblModelAcc = new DefaultTableModel();
                    tblModelAcc.setRowCount(0);

                    // Duyệt qua danh sách thongKe
                    for (int i = 0; i < thongKe.size(); i++) {
                        Vector v = new Vector();
                        v.add(i + 1);                          
                        v.add(thongKe.get(i).getMaSP());        
                        v.add(thongKe.get(i).getTenSP());       
                        v.add(thongKe.get(i).getSlNhap());      
                        v.add(thongKe.get(i).getSlXuat());     
                        tblModelAcc.addRow(v);
                    }
